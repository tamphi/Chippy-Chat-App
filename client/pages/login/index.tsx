  ...
  const buttonController = async (e: React.SyntheticEvent) => {
    ...
    try{
      ...
      if (response.ok){
        ...
        // todo: reload to display all users but self --> done
        location.reload()
      ...
      <form className='flex flex-col md:w-1/5'>
        ...
        >
          ...
          <Link className='flex flex-col md:w-1/5' href="/signup">
            <button className='p-1 mt-0 rounded-md bg-white text-sm text-black text-left'>
              New to Chippy? Click here to signup
            </button>
          </Link>
